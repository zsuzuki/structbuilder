...
void Test::setLUA(sol::state &lua) {
  lua.new_usertype<Note>("TestNote", "page", &Note::page, "line", &Note::line);
  lua.new_usertype<Child>(
      "TestChild", "age", sol::property(&Child::getAge, &Child::setAge), "step",
      sol::property(&Child::getStep, &Child::setStep), "name", &Child::name);
  lua.new_usertype<Entry>("TestEntry", "name", &Entry::name, "country",
                          &Entry::country);
  lua.new_usertype<Test>(
      ...
      "line", &Test::line, "note", &Test::note, "child", &Test::child,
